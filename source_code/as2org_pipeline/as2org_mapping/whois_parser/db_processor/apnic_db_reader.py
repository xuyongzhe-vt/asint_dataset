...
from as2org_pipeline.as2org_mapping.whois_parser.db_processor.util.whois_db_util import read_as_obj_from_item, read_org_obj_from_item, read_contact_obj_from_item, has_only_one_key, read_items_whois_db_in_batch
as_keywords = {'aut-num'}
...
def process_read_items(items):
    ...
    for item in items:
        ...
        if 'aut-num' in item:
            ...
            as_list.append(read_as_obj_from_item(item, aut_num, 'descr', 'admin-c', 'tech-c', 'source', 'org', 'mnt-by', 'NON-EXIST', 'NON-EXIST', 'NON-EXIST', 'as-name', 'abuse-c', 'apnic', changed_time_process_func))
        elif 'organisation' in item:
            org_list.append(read_org_obj_from_item(item, 'admin-c', ['tech-c'], 'phone', 'organisation', 'org-name', 'NON-EXIST', ['NON-EXIST'], 'mnt-ref', 'address', 'NON-EXIST', 'mnt-by', 'apnic', 'org-type', ''))
        elif 'role' in item or 'mntner' in item:
            contact_list.append(read_contact_obj_from_item(item, ['phone', 'fax-no'], 'admin-c', 'tech-c', ['role', 'mntner'], 'address', 'e-mail', 'mnt-by', 'apnic'))
            ...
            as_block_list.append(ASBlock(int(numbers[0]), int(numbers[1]), 'apnic', latest_timestamp, block_assigned_to))
...
def read_apnic_whois_data(filename: str) -> ObjectBundle:
    (as_list, org_list, contact_list, as_block_list) = read_items_whois_db_in_batch(filename, keywords_set, 500000, process_read_items)
    return ObjectBundle(as_list, org_list, contact_list, as_block_list, 'apnic')
