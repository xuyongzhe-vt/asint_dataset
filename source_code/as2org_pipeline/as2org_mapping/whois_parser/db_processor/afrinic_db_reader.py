...
def read_afrinic_whois_data(filename) -> ObjectBundle:
    ...
    for item in items:
        ...
        if 'aut-num' in item:
            ...
            as_list.append(read_as_obj_from_item(item, aut_num, 'descr', 'admin-c', 'tech-c', 'NON-EXIST', 'org', 'mnt-by', 'NON-EXIST', 'notify', 'changed', 'as-name', 'NON-EXIST', 'afrinic', changed_time_process_func))
        elif 'person' in item or 'mntner' in item or 'role' in item:
            contact_list.append(read_contact_obj_from_item(item, ['phone'], 'admin-c', 'tech-c', ['person', 'role', 'mntner'], 'address', 'e-mail', 'mnt-by', 'afrinic'))
        elif 'organisation' in item:
            org_list.append(read_org_obj_from_item(item, 'admin-c', ['tech-c'], 'phone', 'organisation', 'org-name', 'notify', ['NON-EXIST'], 'mnt-ref', 'address', 'changed', 'mnt-by', 'afrinic', '', ''))
            ...
            as_block_list.append(ASBlock(int(numbers[0]), int(numbers[1]), 'afrinic', latest_timestamp, block_assigned_to))
    return ObjectBundle(as_list, org_list, contact_list, as_block_list, 'afrinic')
