...
def compare_dataset(our_family_asn_lists, their_family_asn_lists, asn_to_name, output_dir):
    ...
    def filter_clusters(cluster_list):
        return [
            [asn for asn in cluster if asn in common_asns]
            for cluster in cluster_list
            if any(asn in common_asns for asn in cluster)
        ]
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"Our clusters fully covering theirs: {len(fully_covered_map)} "
            f"(multi={cover_counts_ours['multi']}, single={cover_counts_ours['single']}, "
            f"ASNs influenced={asns_covered_by_ours})\n"
        )
        f.write(
            f"Their clusters fully covering ours: {len(reverse_fully_covered_map)} "
            f"(multi={cover_counts_theirs['multi']}, single={cover_counts_theirs['single']}, "
            f"ASNs influenced={asns_covered_by_theirs})\n"
        )
    ...
    _write_cover_file(
        os.path.join(output_dir, "our_cover.txt"),
        fully_covered_map, asn_to_name, header_ours=True,
    )
    _write_cover_file(
        os.path.join(output_dir, "reverse_cover.txt"),
        reverse_fully_covered_map, asn_to_name, header_ours=False,
    )
...
def _write_cover_file(path, cover_map, asn_to_name, header_ours):
    ...
    with open(path, "w", encoding="utf-8") as f:
        for outer_cluster, inner_clusters in sorted(
            cover_map.items(), key=lambda kv: len(kv[0]), reverse=True
        ):
