...
MODEL_ORDER = [
    ("deepseek_r1_distill_32b", "DeepSeek-R1-Distill-32B", 8),
    ("qwen25_72b", "Qwen2.5-72B", 2),
    ("qwen25_32b", "Qwen2.5-32B", 8),
    ("gpt_oss_20b", "gpt-oss-20b", 4),
    ("qwen25_7b", "Qwen2.5-7B", 30),
    ("qwen25_3b", "Qwen2.5-3B", 50),
    ("qwen25_1_5b", "Qwen2.5-1.5B", 40),
    ("qwen25_0_5b", "Qwen2.5-0.5B", 100),
    ("deepseek_ablation_no_context", "DS-32B (no context)", 8),
    ("deepseek_ablation_minimal_prompt", "DS-32B (minimal prompt)", 8),
]
...
def score(preds, gold):
    ...
    return dict(tp=tp, fp=fp, fn=fn, tn=tn, loops=loops,
                n_total=n_total, n_valid=n_valid,
                failure_rate=loops / n_total,
...
def main():
    ...
    md = [
        "# Final results — loops excluded from P/R/F1, reported as failure rate",
        "",
        f"Benchmark: n={len(gold)} (pos={sum(1 for g in gold.values() if g['gold_label'] in POS)}, "
        f"neg={sum(1 for g in gold.values() if g['gold_label'] not in POS)})",
        "",
        "| Model | Conc | Failure rate | P | R | F1 | Acc | Pairs/s |",
        "|---|---|---|---|---|---|---|---|",
    ]
    for d, m in rows:
        tp = f"{m['system_throughput']:.2f}" if m['system_throughput'] else "—"
        md.append(
            f"| **{d}** | {m['conc']} | {m['failure_rate']*100:.1f}% ({m['loops']}/{m['n_total']}) | "
    ...
    with open(OUT_DIR / "final_table.csv", "w", newline="") as f:
        ...
        w.writerow(["model", "concurrency", "failure_rate_pct", "loops", "n_total",
                    ...
                    "TP", "FP", "FN", "TN"])
        for d, m in rows:
            w.writerow([d, m["conc"], f"{m['failure_rate']*100:.2f}", m["loops"], m["n_total"],
                        f"{m['precision']*100:.2f}", f"{m['recall']*100:.2f}",
                        f"{m['f1']*100:.2f}", f"{m['accuracy']*100:.2f}",
                        f"{m['avg_latency_s']:.2f}" if m['avg_latency_s'] is not None else "",
                        f"{m['system_throughput']:.2f}" if m['system_throughput'] is not None else "",
