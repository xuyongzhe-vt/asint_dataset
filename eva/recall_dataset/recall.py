...
\
    ...
    universe = set().union(*clusters) if clusters else set()
    asn_to_cluster = {}
    ...
    for org, gt_asns in gt.items():
        gt_common = gt_asns & universe
        if not gt_common:
            rows.append((org, len(gt_asns), 0, 0, 0.0))
            continue
        ...
        if not cluster_overlap:
            rows.append((org, len(gt_asns), len(gt_common), 0, 0.0))
            continue
        best_ci, best_overlap = max(cluster_overlap.items(), key=lambda kv: kv[1])
        recall = best_overlap / len(gt_common)
        rows.append((org, len(gt_asns), len(gt_common), best_overlap, recall))
    return rows
